...
`tm_profile()` turns a genome, or any set of regions in it, into a melting
temperature profile: it tiles the regions into windows, fetches each
window's sequence, computes a Tm, and returns one `GRanges`. One call does
what would otherwise take three, and on a mammalian genome it spreads the
...
`tm_profile()` to run in a single process.
...
The smallest useful call needs a genome and nothing else:
...
and computes a nearest-neighbour Tm for each, in a single process. Adding
workers is one more argument:

```{r quick-start-parallel}
library(BiocParallel)

tm <- tm_profile(hg38, BPPARAM = SnowParam(workers = 5))
```
...
```{r sources}
tm_profile(hg38)                       # an installed BSgenome package
tm_profile("contigs.fa.gz")            # a FASTA file, gzip is fine
tm_profile(oligos)                     # a character vector of sequences
```
...
tm_profile(hg38, regions = 1:22)                                # autosomes
...
## call for: a FASTA of array probes, primers or synthetic oligos returns one
## Tm per record. It is the default when sequences are passed directly.
tm_profile("probes.fa", window = NULL)
tm_profile(oligos, BPPARAM = SnowParam(5))
```

Three things are worth knowing before you trust the output.

*The default includes chrM.* With no `regions`, `tm_profile()` covers
...
in the result, so `tm_profile()` warns rather than double-counting them
quietly.
...
`bench_tm_profile.R` in the same directory times `tm_profile()` itself over
a range of worker counts, with `bench_tm_profile.lsf` to submit it, and is
...
# When to use `tm_calculate()` instead

`tm_profile()` starts from coordinates in a source it can open. When the
sequences are already in hand, and there are few enough that parallelism
would not pay, use `tm_calculate()`:
...
construction, sequence retrieval and result assembly run once in the calling
process, and sending the sequences to workers costs more than the loop it
divides. Measured on chromosome 1, splitting one call across five workers
was never faster than the serial call. Parallelism belongs at the level of
regions, which is what `tm_profile()` does.
