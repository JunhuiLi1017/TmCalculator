...
```{r zoom-multi-linear, fig.width=10, fig.height=9, fig.cap="Two zoomed regions, 0.1-0.5 Mb and 3.6-4.5 Mb, concatenated around the circle with a gap between them."}
plot_genome_track(
  genome_name = genome_name,
  genome_size = chr_length,
  track_list  = tracks,
...
## Largest Tm difference between two windows of identical GC inside a locus.
best_pair <- function(d) {
...
sc <- vapply(starts, function(i) {
  b <- best_pair(W[i:(i + n_loc - 1L), ]); if (is.null(b)) 0 else b$spread
}, numeric(1))

i0   <- starts[which.max(sc)]
d    <- W[i0:(i0 + n_loc - 1L), ]
pair <- d[best_pair(d)$rows, ]
pair[, c("seqnames", "start", "end", "GC", "Tm")]
```

```{r zoom-locus, fig.width=8, fig.height=4, fig.cap="High-magnification view of the locus containing the largest melting-temperature difference between two windows of identical GC content. The two windows are shaded. Only the GC and Tm tracks are drawn: seven tracks in one linear panel leave each of them too little height to read at this scale."}
tracks_zoom <- list(
  list(type = "line", data = Tm, value_col = "GC", name = "GC content",
       col = "#4A90E2", legend_font_col = "#4A90E2", height = 1),
  list(type = "line", data = Tm, value_col = "Tm", name = "Melting temp",
       col = "#E06666", legend_font_col = "#E06666", height = 1.4),
  list(type = "highlight", data = pair[, c("seqnames", "start", "end")],
       col = "#B7791F", alpha = 0.28)
)

plot_genome_track(
  ## The chromosome bar is labelled with `genome_name`, and the BSgenome
  ## package name is too long for a 20 kb panel; the accession is short and
  ## says the same thing.
  genome_name = chr_name,
  genome_size = chr_length,
...
The GC track is flat across the two shaded windows and the Tm track is not.
That is the whole argument for a nearest-neighbour model in one picture, and
it is the panel used as Figure 2C in the manuscript; the combined figure is
produced by `inst/scripts/make_figure2.R`.

One caveat worth stating plainly: this locus was selected *because* it is
extreme, so it shows the largest effect rather than a typical one. The
box plot above is the honest summary, and the two belong together.
