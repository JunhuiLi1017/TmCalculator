...
1. **Get a reference genome** as a BSgenome package: use the one Bioconductor
   ships for your assembly, or forge one when there is none.
2. **Compute the Tm profile** for every window with `tm_profile()`, which
   tiles the genome, retrieves the sequence and computes Tm in one call.
3. **Visualise** the profile with `plot_genome_track()`: circular plots,
   linear plots, zoom views, and multi-omics overlays.
4. **Test** whether Tm inside MutL-AR peaks differs from the rest of the
   genome, with `compare_groups()`.
...
# Step 1 — The reference genome {#build-bsgenome}

`tm_profile()` retrieves sequence from a BSgenome package, so the first
...
Step 2; nothing in this vignette needs the genome to have been built by hand.
...
# Step 2 — Compute the genome-wide Tm profile

`tm_profile()` does the whole of it in one call: it tiles the requested
...
runtime <- system.time({
  tm_ecoli <- tm_profile(
    seq_source = genome_name,  # a BSgenome package name, or a FASTA path
...
mammalian genome the same call takes a `BPPARAM` and spreads the tiles
over workers; `vignette("hg38_performance_parallel")` covers that, together
with the forms `regions` accepts and how to choose a worker count.

For sequences that are already in hand, as character vectors, a FASTA file
or a `GRanges` carrying a `sequence` column, use `tm_calculate()` instead:
`tm_profile()` is the entry point for coordinates in a reference genome,
and `tm_calculate()` for sequences.

---

# Step 3 — Visualise with `plot_genome_track()`
...
tm_gc_ecoli <- tm_profile(genome_name, regions = chr_name, unit = "region",
...
three-panel figure it feeds (Figure 2 of the manuscript), are in
`inst/scripts/make_figure2.R`, which is shipped with the package:

```{r make-figure2, eval=FALSE}
script <- system.file("scripts", "make_figure2.R", package = "TmCalculator")
...
|------|----------|---------------|
| Tiling, sequence retrieval and Tm for 23,208 windows | `tm_profile()` | ~`r sprintf("%.2f", runtime[["elapsed"]])` s |
