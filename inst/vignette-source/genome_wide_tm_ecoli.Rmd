...
window along the single circular chromosome (the bin size used by
Hasenauer *et al.* 2025). A window-size sensitivity
analysis (50, 100, 200 and 500 bp) shows that absolute Tm scales with
window length as expected from duplex thermodynamics, while the spatial
landscape (cross-scale correlations >= 0.975) and the MutL-AR association
(~1 degree C lower median Tm, ~2.7 percentage points lower GC in peaks,
all p < 1e-12) are reproduced at every window size.
...
|------|----------|---------------|
| Window generation | `make_genomiccoord()` | ~0.1 s |
| Sequence extraction | `to_genomic_ranges_fast()` | ~0.5 s |
| Tm calculation (23,208 windows) | `tm_calculate()` | ~0.6 s |
| **Total** | | **~1.2 s** |
