...
#
#   Rscript inst/scripts/make_figure2.R --in-peaks
#   Rscript inst/scripts/make_figure2.R --n-regions 1 --n-groups 4
#   Rscript inst/scripts/make_figure2.R --region "U00096.3:212801-232800"
#
...
# target is four intervals across the view, matching Figure 3, which spans
...
cache     <- argval("--cache", file.path(outdir, "figure2_profiles.rds"))
...
## -- Write -----------------------------------------------------------------
stem <- file.path(outdir, "figure2_ecoli_circos_zoom")
...
info <- file.info(list.files(outdir, pattern = "^figure2_.*\\.(pdf|tif|jpg)$",
                             full.names = TRUE))
cat("\nWritten:\n")
